...
def train(train_type = 'default',
    ...
    df_train = open_df(path, 'train_genie_equal')
    ...
    df_val = open_df(path, 'val_genie_equal')
    ...
    if train_type == 'dann':
        mobnetmodel = scripts.mobilenetv2.MobileNetV2_DANN(input_shape=((2, 80, 100),), classifier_classes=3, descriminator_classes = 2, DANN_strength = DANN_strength)
        mobnetmodel.compile(optimizer=opt, loss='categorical_crossentropy',metrics=['accuracy'])

    elif train_type== 'default':
        mobnetmodel = scripts.mobilenetv2.MobileNetV2(input_shape=((2, 80, 100),), classes=3)
        mobnetmodel.compile(optimizer=opt, loss='categorical_crossentropy',metrics=['accuracy'])

    elif train_type == 'descr':
        mobnetmodel = scripts.mobilenetv2.MobileNetV2(input_shape=((2, 80, 100),), classes=2)
...
def test(weights_file, path, name, dataset_percent = 0.1, data = 'both', model_type = 'default', output = 'default'):
    ...
    model.load_weights('/home/ishokar/april_test/' + weights_file)
    ...
    if model_type != 'dann':
        ...
        df2, list_out = index_finder(probabilities, df_row)
        pkl.dump(df2, open('df_physics.pkl','wb'))
        okl.dump(list_out, open('physics_list.pkl', 'wb'))
