...
def index_finder(probabilities, df_row):
    ...
    for index, evt in enumerate(df['evt']):
        ...
        row.append(df.loc[index][-1])
        print(row)
        df_out.loc[index] = row
        print(round(index/len(df['evt']),3))
        with open('physics_data.csv', "a") as csv_file:
            writer = csv.writer(csv_file, delimiter=',')
            writer.writerow(row)

    return df_out, list_out
